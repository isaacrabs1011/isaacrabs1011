...
# Authenticate with Spotify using client credentials flow (doesn't require user login)
auth_manager = SpotifyClientCredentials(SPOTIPY_CLIENT_ID, SPOTIPY_CLIENT_SECRET)
isaac = spotipy.Spotify(auth_manager=auth_manager)  # Create a Spotify API client using the authenticated session

# Fetch playlist details using its Spotify URL (or ID)
playlist = isaac.playlist('https://open.spotify.com/playlist/6GhmUa261lqAmWsod8A6vu?si=4d354f348327435b')
